...
class Dataset(object):
    ...
    def __init__(self,
        ...
        self.dim = 3
        self.image_base_folder = image_base_folder or os.path.join(self.base_folder, 'images_reoriented_smoothed_0_75')
    ...
    def intensity_preprocessing_ct(self, image):
        ...
        return image #gaussian_sitk(image, self.input_gaussian_sigma)
    ...
    def intensity_preprocessing_ct_random(self, image):
        ...
        """
        image = ShiftScaleClampSitk(clamp_min=-1024)(image)
        # if self.crop_image_top_bottom:
        #     spacing_z = image.GetSpacing()[2]
        #     # set 25 mm on top and bottom of image to 0
        #     size = 25
        #     image = sitk.Crop(image, [0, 0, int(spacing_z * size)], [0, 0, int(spacing_z * size)])
        return image #gaussian_sitk(image, self.input_gaussian_sigma)
    ...
    def dataset_train(self):
        ...
        if self.use_variable_image_size:
            #image_size = ImageSizeGenerator(self.dim, [None] * 3, self.image_spacing, valid_output_sizes=[self.valid_output_sizes_x, self.valid_output_sizes_y, self.valid_output_sizes_z], name='output_size', kwparents={'image': sources['image']})
        ...
        #landmark_sanity_check = LandmarkSanityCheckNode(utils.io.text.load_dict_csv(self.valid_landmarks_file), utils.io.text.load_list_csv(self.train_file), name='valid_landmarks_histogram', parents=[iterator, generators['landmarks']])

        return GraphDataset(data_generators=list(generators.values()),# + [landmark_sanity_check],
