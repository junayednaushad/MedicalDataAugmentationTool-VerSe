...
class MainLoop(MainLoopBase):
    ...
    def bb(self, image, transformation):
        image_thresholded = (np.squeeze(image / np.max(image)) > 0.25).astype(np.uint8)
        ...
        start = np.flip(start.astype(np.float64) * np.array(self.image_spacing, np.float64))
        end = np.flip(end.astype(np.float64) * np.array(self.image_spacing, np.float64))
    ...
    def test(self):
        ...
        for current_id in tqdm(self.image_id_list, desc='Testing'):
            try:
                ...
                start, end = self.bb(prediction, transformation)
                bbs[current_id] = start + end

                if self.save_output_images:
                    origin = np.array(transformation.TransformPoint(np.zeros(3, np.float64)))
                    utils.io.image.write_multichannel_np(image, self.output_folder_handler.path('output', current_id + '_input.mha'), output_normalization_mode='min_max', data_format=self.data_format, image_type=np.uint8, spacing=self.image_spacing, origin=origin)
                    utils.io.image.write_multichannel_np(prediction, self.output_folder_handler.path('output', current_id + '_prediction.mha'), output_normalization_mode=(0, 1), data_format=self.data_format, image_type=np.uint8, spacing=self.image_spacing, origin=origin)
            except:
