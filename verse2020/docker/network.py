...
class SpatialConfigurationNet(tf.keras.Model):
    ...
    def __init__(self,
        ...
        self.unet = DenseUnet3D if dense_unet else UnetAvgLinear3D
        self.data_format = data_format
        self.num_filters_base = num_filters_base
        if activation == 'relu':
...
class Unet(tf.keras.Model):
    ...
    The SpatialConfigurationNet.
    """
    def __init__(self, num_labels, num_filters_base=64, num_levels=4, activation='relu', data_format='channels_first', padding='same', dropout_ratio=0.0, heatmap_initialization=False, **kwargs):
        """
        The spatial configuration net.
        :param input: Input tensor.
        :param num_labels: Number of outputs.
